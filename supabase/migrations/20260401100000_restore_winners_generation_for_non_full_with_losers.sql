...
begin
  ...
  if v_is_full_with_losers and v_n >= 4 and v_n <= 16 then
    ...
    for v_round in 1..(v_rounds-1) loop
      ...
      for v_slot in 1..v_matches_in_round loop
        insert into public.matches(tournament_id, stage, bracket_type, round, bracket_slot)
        select p_tournament_id, 'PLAYOFF', 'LOSERS', v_drop_round, v_slot
        where not exists (
          select 1
          from public.matches mx
          where mx.tournament_id = p_tournament_id
      ...
      if v_round < (v_rounds - 1) then
        ...
        for v_slot in 1..v_matches_in_round loop
          insert into public.matches(tournament_id, stage, bracket_type, round, bracket_slot)
          select p_tournament_id, 'PLAYOFF', 'LOSERS', v_drop_round + 1, v_slot
          where not exists (
            select 1
            from public.matches mx
            where mx.tournament_id = p_tournament_id
    ...
    insert into public.matches(tournament_id, stage, bracket_type, round, bracket_slot)
    select p_tournament_id, 'PLAYOFF', 'LOSERS', v_loser_rounds + 1, 1
    where not exists (
      select 1
      from public.matches mx
      where mx.tournament_id = p_tournament_id
        and mx.stage = 'PLAYOFF'
        and mx.bracket_type = 'LOSERS'
        and mx.round = v_loser_rounds + 1
        and mx.bracket_slot = 1
    );

    insert into public.matches(tournament_id, stage, bracket_type, round, bracket_slot)
    select p_tournament_id, 'PLAYOFF', 'LOSERS', v_loser_rounds + 2, 1
    where not exists (
      select 1
      from public.matches mx
      where mx.tournament_id = p_tournament_id
