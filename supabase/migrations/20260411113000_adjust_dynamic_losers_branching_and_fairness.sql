...
begin
  ...
  if v_is_full_with_losers and v_n >= 4 and v_n <= 16 then
    ...
    -- Keep placement bracket dynamic for full_with_losers:
    -- losers-round shells are created lazily when corresponding winners matches lock.
    -- Only grand final game 1 is pre-created so winners-final progression has a target.
    v_gf1_round := v_rounds;
