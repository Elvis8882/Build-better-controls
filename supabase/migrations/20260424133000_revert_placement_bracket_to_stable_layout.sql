-- Revert placement-bracket DB behavior to stable branch while preserving
-- mirrored WB->LB seeding and 8-participant 7th/8th placement match handling.
...
begin
  ...
  if v_is_full_with_losers and v_n >= 3 and v_n <= 16 then
    ...
    -- Remaining WB rounds alternate:
    -- consolidation of existing LB survivors -> intake of new WB losers.
    for v_round in 2..v_rounds loop
      if v_lb_prev_winners > 1 then
        v_lb_round_count := v_lb_round_count + 1;
        v_lb_round := v_lb_round_count;
