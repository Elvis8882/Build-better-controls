...
begin
  ...
  -- Idempotent loser ingestion keyed by winners source match.
  -- If a locked result is edited/relocked, we overwrite the prior loser mapping for that source match.
  ...
  if v_preset = 'full_with_losers' and v_max_round >= 3 then
    ...
    if v_source_stage = 'SF' then
      ...
      -- Populate round-2 side match only when an actual round-1 winner exists for this side.
      -- With odd participant counts this side can legitimately have no round-1 placement match.
      select id into v_round1_match_id
      from public.matches
      where tournament_id = m.tournament_id
        and stage = 'PLAYOFF'
      ...
      if v_round1_match_id is not null then
        select case
      ...
      -- Third-place match is always driven by actual semifinal losers.
      -- QF-loser placement matches (5-8 path) are optional and must not gate 3rd place.
      if exists (
        select 1
        from public.playoff_placement_entrants e
        where e.tournament_id = m.tournament_id
        ...
        if v_round3_match_id is not null and v_home is not null and v_away is not null then
          update public.matches
          set home_participant_id = v_home,
              away_participant_id = v_away,
              next_match_id = null,
              next_match_side = null
          where id = v_round3_match_id;

          perform public.sync_match_identities_from_participants(v_round3_match_id);
        end if;
      end if;
