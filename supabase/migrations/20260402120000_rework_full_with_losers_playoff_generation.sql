...
begin
  ...
  if v_is_full_with_losers and v_n >= 4 and v_n <= 16 then
    ...
    -- Build deterministic seeded occupancy map (BYE-aware), then compute how many
    -- real losers drop from each winners round.
    ...
    -- Round 1 of losers receives winners round-1 losers.
    ...
    for v_round in 2..v_rounds loop
      -- Drop round: LB winners + new WB losers.
      ...
      -- Consolidation between drop rounds (except after WB final drop).
    ...
    -- Materialize losers rounds, including explicit odd-entry auto-advance nodes
    -- as real matches (ceil(entrants/2) rule).
    ...
    -- LB champion feeds GF1 as AWAY, WB champion feeds GF1 as HOME.
