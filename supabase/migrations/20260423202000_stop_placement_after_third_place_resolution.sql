-- Stop placement bracket at third-place resolution (no WB final loser drop).
...
begin
  ...
  -- - Build losers rounds using alternating intake/consolidation phases:
  --     L1: WB R1 loser intake
  --     L2: consolidation of L1 survivors
  --     L3: intake from WB R2 losers + L2 survivors
  --     L4: consolidation
  --     ...
  --   (Small fields can collapse empty phases naturally.)
  --
  -- Size notes:
  ...
  if v_is_full_with_losers and v_n >= 3 and v_n <= 16 then
    ...
    if v_lb_entrants > 0 then
      ...
      v_drop_round_by_wb[1] := v_lb_round;
      v_lb_prev_winners := ceil(v_lb_entrants / 2.0)::int;
    end if;

    -- Remaining WB rounds (excluding the winners final) are built as intake-first:
    -- inject losers from the just-finished WB round, then only consolidate
    -- if additional reduction is needed before the next WB loser intake.
    for v_round in 2..greatest(v_rounds - 1, 1) loop
    ...
    -- Placement bracket stops before winners final loser drops.
    -- No grand-final reset nodes are created in LOSERS for this flow.
    ...
    for v_round in 1..(v_lb_round_count - 1) loop
      for v_slot in 1..v_lb_matches[v_round] loop
        ...
        v_parent_matches := coalesce(v_lb_matches[v_round + 1], 0);
        if v_parent_matches = v_lb_matches[v_round] then
          v_to_slot := v_slot;
        else
    ...
    -- This is full_with_losers-only and ensures consolidation winners do not
    ...
    -- No LOSERS continuation after placement final.
  else
  ...
  if v_has_group then
    -- Group-stage seeding is ordered by placement within each group first
    -- (1A,1B,2A,2B,...) so cross-group first-round pairings are preserved.
    select array_agg(s.participant_id order by s.seed asc) into v_seeded
    from public.v_playoff_seeds s
